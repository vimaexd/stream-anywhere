...
impl EventHandler for Handler {
  ...
  async fn message(&self, ctx: Context, message: Message) {
    ...
    let urls: Vec<&String> = all_urls
      .iter()
      .filter(|url| {
        SUPPORTED_URLS.iter().any(|supported_url| url.starts_with(supported_url))
      })
      .collect();
    ...
    for url in urls {
      ...
      let spotify_data = song_info.entities_by_unique_id
      ...
      content.push_str(format!("## {} - {} \n", spotify_data.artist_name.as_ref().unwrap(), spotify_data.title.as_ref().unwrap()).as_str());
      ...
      for service in DISPLAY_SERVICES.iter() {
        ...
        let separator = if DISPLAY_SERVICES.iter().position(|x| x == service).unwrap() == DISPLAY_SERVICES.len() - 1 { "" } else { "- " };

        content.push_str(format!("[{}](<{}>) {}", 
          format!("{}{}", service.chars().next().unwrap().to_uppercase(), &service[1..]) , 
          platform_info.url, 
          separator
        ).as_str());
